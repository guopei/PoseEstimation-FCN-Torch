...
--paths.dofile('../ReLU1.lua')

local function hourglass(n, f, inp, opt)
    -- Upper branch
    local up1 = inp
    ...
    if n > 1 then low2 = hourglass(n-1,f,low1,opt)
    else
        low2 = low1
...
local function visi(tmpOut, opt)
    local scale = math.floor(opt.outputRes / 64)
    local subnet = nn.Sequential()
        :add(nn.View(-1, opt.keypointNum, 64*64))
        :add(nn.Sum(3))
        :add(nn.View(-1,opt.keypointNum))
        :add(nn.SplitTable(2))
        :add(nn.MapTable():add(nn.Sigmoid()))

    local joint = nn.ConcatTable():add(nn.SpatialUpSamplingBilinear(scale)):add(subnet)(tmpOut)
    return joint
end
...
function createModel(opt)
    ...
    for i = 1,opt.nStack do
        local hg = hourglass(4,opt.nFeats,inter,opt)
        ...
        -- Predicted heatmaps
        local tmpOut = nnlib.SpatialConvolution(opt.nFeats,opt.keypointNum,1,1,1,1,0,0)(ll)
        --local joint = visi(tmpOut, opt)
        ...
        if i < opt.nStack then
            ...
            local tmpOut_ = nnlib.SpatialConvolution(opt.keypointNum,opt.nFeats,1,1,1,1,0,0)(tmpOut)
