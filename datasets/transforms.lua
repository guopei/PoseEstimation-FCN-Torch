...
-- Scales to (size, size)
function M.UniformScale(size, interpolation)
    interpolation = interpolation or 'bicubic'
    return function(input)
        input = image.scale(input, size, size, interpolation)
        -- what's this!!! remove it!!!!
        --input[1]:mul(torch.uniform(0.6,1.4)):clamp(0,1)
        --input[2]:mul(torch.uniform(0.6,1.4)):clamp(0,1)
        --input[3]:mul(torch.uniform(0.6,1.4)):clamp(0,1)
        return input
    end
end
...
function M.Rotation(deg)
    return function(input)
        local maxNum = #deg
        local randInd = torch.random(maxNum)
        if deg[randInd] == 0 then
            return input 
        else
            input = image.rotate(input, deg[randInd] * math.pi / 180, 'bilinear')
            return input, deg[randInd]
        end
    end
end
